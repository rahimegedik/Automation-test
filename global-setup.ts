...
async function globalSetup(_config: FullConfig) {
  ...
  const DESTRUCTIVE_SPECS = [
    ...
    '12-2-ayda-1-birikim', '13-3-ayda-1-birikim',
    '14-4-ayda-1-birikim', '15-6-ayda-1-birikim',
  ...
  const email = process.env[`TEST_EMAIL_${env.toUpperCase()}`] ?? process.env.TEST_EMAIL;
  const password = process.env[`TEST_PASSWORD_${env.toUpperCase()}`] ?? process.env.TEST_PASSWORD;
  ...
  const apiHost = process.env[`API_HOST_${env.toUpperCase()}`] ?? baseURL.replace('://www.', '://api.');

  page.on('response', resp => {
    if (resp.url().includes(`${apiHost}/customer/login`) && resp.status() === 200) {
  ...
  if (page.url().includes('accounts.google.com') || page.url().includes('auth.nadirgold.work')) {
  ...
  const loginBtn = page.locator('button').filter({ hasText: /^Giriş Yap$/i }).first();
  ...
  await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Login API 30sn içinde yanıt vermedi')), 30_000)
    ),
  ]);
...
async function handleGoogleAuth(page: Page, googleEmail: string, googlePassword: string) {
  for (let step = 0; step < 15; step++) {
    ...
    if (!url.includes('accounts.google.com') && !url.includes('auth.nadirgold.work')) {
      return;
    }

    // Debug ekran görüntüsü
    await page.screenshot({ path: `playwright/.auth/debug-google-${step}.png`, fullPage: true });
    ...
    const pwdVisible = await pwdField.evaluate(
    ...
    const pwdOptionHandle = await page.evaluateHandle(() => {
      ...
      return els.find(el =>
        el.textContent?.trim().match(/şifrenizi girin|enter your password/i) &&
...
async function checkSession(authFile: string, baseURL: string): Promise<boolean> {
