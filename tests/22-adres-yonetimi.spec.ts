...
test.describe("Adres Yönetimi (gerçek silme/ekleme yok)", () => {
  ...
  test('TC-ADDR-004: Adres kartında "Adresi Düzenle" butonu görünür', async ({
    page,
  }) => {
    const addressPage = new AddressPage(page);
    await addressPage.goto();
  ...
  test("TC-ADDR-007: Boş form ile Kaydet → form açık kalır", async ({
    page,
  }) => {
    const addressPage = new AddressPage(page);
    await addressPage.goto();
  ...
  test('TC-ADDR-008: "Alıcı Ekle (İsteğe Bağlı)" butonu form içinde mevcut', async ({
    page,
  }) => {
    const addressPage = new AddressPage(page);
    await addressPage.goto();
  ...
  test("TC-ADDR-009: Fatura tipi radio butonları mevcut", async ({ page }) => {
    ...
    console.log(
      `✓ ${radioCount} radio button görünüyor (bireysel/kurumsal fatura tipi)`,
    );
  });
});
