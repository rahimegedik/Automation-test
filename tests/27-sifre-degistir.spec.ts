...
test.describe('Şifre Değiştirme (gerçek değişim yok)', () => {
  ...
  test('TC-PWD-002: 3 password input + Güncelle butonu görünür', async ({ page }) => {
    const pwdPage = new PasswordChangePage(page);
    await pwdPage.goto();
    await expect(pwdPage.oldPasswordInput).toBeVisible();
  ...
  test('TC-PWD-005: Tüm password input\'ları type="password"', async ({ page }) => {
    const pwdPage = new PasswordChangePage(page);
    await pwdPage.goto();
    expect(await pwdPage.oldPasswordInputType()).toBe('password');
    expect(await pwdPage.newPasswordInputType()).toBe('password');
    expect(await pwdPage.confirmPasswordInput.getAttribute('type')).toBe('password');
  ...
  test('TC-PWD-006: Boş form + Güncelle → engellenir', async ({ page }) => {
    ...
    let apiCalled = false;
    page.on('request', req => {
      if (/(password|change|sifre)/i.test(req.url()) && req.method() !== 'GET') {
        apiCalled = true;
      }
    });
    ...
    await expect(page).toHaveURL(/\/hesabim\/sifre-degistir/);
    await expect(pwdPage.oldPasswordInput).toBeVisible();
    console.log(`✓ Boş form: API çağrısı=${apiCalled} (engellenmeli), form açık`);
  });

  test('TC-PWD-003: Hatalı mevcut şifre → değişiklik reddedildi', async ({ page }) => {
    const pwdPage = new PasswordChangePage(page);
    await pwdPage.goto();
    const status = await trackPasswordChangeAttempt(page, () =>
      pwdPage.fillForm('YANLIS-MEVCUT-SIFRE-123', 'YeniSifre123!', 'YeniSifre123!').then(() =>
        pwdPage.submit()
      )
    );
    ...
    await expect(page).toHaveURL(/\/hesabim\/sifre-degistir/);
    console.log(`✓ Hatalı mevcut şifre reddedildi (API status: ${status ?? 'çağrılmadı'})`);
  });

  test('TC-PWD-004: Yeni şifre + Tekrar eşleşmezse → form açık kalır', async ({ page }) => {
    const pwdPage = new PasswordChangePage(page);
    await pwdPage.goto();
    let apiCalled = false;
    page.on('request', req => {
    ...
    await pwdPage.fillForm('AnyCurrentPwd!', 'YeniSifre123!', 'FARKLI-TEKRAR-456');
    await pwdPage.submit();
    await page.waitForTimeout(2000);
    await expect(page).toHaveURL(/\/hesabim\/sifre-degistir/);
    await expect(pwdPage.oldPasswordInput).toBeVisible();
    console.log(`✓ Şifre eşleşmeme: API çağrısı=${apiCalled} (client validation engellemeli)`);
  });
});
