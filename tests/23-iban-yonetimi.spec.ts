...
test.describe("IBAN Yönetimi (gerçek silme yok)", () => {
  ...
  test("TC-IBAN-002: IBAN kartları listelenir", async ({ page }) => {
    ...
    console.log(
      `✓ ${count} adet IBAN kartı görünüyor (en az 1 olabilir, hesaba bağlı)`,
    );
  ...
  test('TC-IBAN-005: "Yeni IBAN Ekle" formu açılır', async ({ page }) => {
    ...
    console.log(
      "✓ Yeni IBAN Ekle formu açıldı (bank/account/iban/save görünür)",
    );
  });
  ...
  test("TC-IBAN-006/008: Sil butonu görünür (gerçek silme yok)", async ({
    page,
  }) => {
    const ibanPage = new IbanPage(page);
    await ibanPage.goto();
