...
test.describe('Kayıtlı Kartlar (UI assert only — yıkıcı değil)', () => {
  ...
  test('TC-CARD-002: Başlık görünür', async ({ page }) => {
    ...
    const count = await page.getByText('Kayıtlı Kartlarım', { exact: true }).count();
  ...
  test('TC-CARD-004: Kart bilgileri (masked numara + SKT)', async ({ page }) => {
    const cardsPage = new CardsPage(page);
    await cardsPage.goto();
    const count = await cardsPage.cardCount();
    test.skip(count === 0, 'Kayıtlı kart yok, bu test için gerçek kart gerekli');

    expect(await cardsPage.hasMaskedNumber(0)).toBe(true);
    expect(await cardsPage.hasExpiry(0)).toBe(true);
    console.log(`✓ Kart 0: masked numara + SKT görünüyor (toplam ${count} kart)`);
  });
  ...
  test('TC-CARD-005: Banka fallback ("Bankanız")', async ({ page }) => {
    ...
    if (!fallback) {
      ...
      console.log(`ℹ Bu kartta gerçek banka adı var, fallback testi N/A. Kart text: ${text.slice(0, 60)}`);
      test.skip();
    }
  ...
  test('TC-CARD-006: Sil butonu görünür (gerçek silme yok)', async ({ page }) => {
    const cardsPage = new CardsPage(page);
    await cardsPage.goto();
    const count = await cardsPage.cardCount();
    test.skip(count === 0, 'Kayıtlı kart yok');
    ...
    const trashButtons = page.locator('button:has(svg path[d*="M"])')
