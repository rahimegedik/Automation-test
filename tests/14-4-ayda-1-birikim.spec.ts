...
test('Düzenli Birikim Akışı - 4 Ayda 1', async ({ page }) => {
    ...
    await test.step('4. GRAM KÜLÇE ALTIN kategorisine git', async () => {
        await page.getByRole('link', { name: 'GRAM KÜLÇE ALTIN', exact: true }).click();
        await expect(page.locator('body')).toBeVisible();
    ...
    await test.step('12. 4 Ayda 1 düzenli birikim talimatını seç', async () => {

        await page.getByPlaceholder('Talimat Adı Giriniz')
            .fill(`test-4-ayda-1-${Date.now()}`);

        await page.waitForTimeout(500);

        // Slider'ı en sola resetle, sonra hedefe git (Her Ay → 2 → 3 → 4 → 6)
        // Hedef: 4 Ayda 1 = offset 3
        ...
        for (let attempt = 0; attempt < 10; attempt++) {
            ...
            if (currentText.includes('4 Ayda 1')) break;
            await sliderHandle.press('ArrowRight');
            await page.waitForTimeout(300);
        }
        await page.waitForTimeout(500);

        await expect(page.locator('.rc-slider')).toContainText('4 Ayda 1');
        ...
        console.log('✓ 4 Ayda 1 düzenli birikim seçildi');
    });
    ...
    await test.step('18. Siparişlerim / Birikim Planlarım sayfasına bak', async () => {
        ...
        const hasPlan = await recurringPlan
            .then(text => text?.toLowerCase().includes('birikim') || text?.toLowerCase().includes('recurring'))
            .catch(() => false);

        console.log(`✓ Siparişlerim sayfası açıldı${hasPlan ? ' - Birikim planı görülüyor' : ''}`);
    });
