...
async function attemptLogin(
  ...
  const onResp = (resp: import("@playwright/test").Response) => {
    if (
      resp.url().includes("/customer/login") &&
...
function expectFailedLogin(status: number | null): void {
  ...
  if (status !== null) {
    expect(
      status,
      `Login API'den 4xx beklendi, ${status} geldi`,
...
test.describe("Login negatif + UI testleri", () => {
  test("TC-LOG-005: Kayıtsız email → giriş reddedildi", async ({ page }) => {
    ...
    console.log(
      `✓ Kayıtsız email reddedildi (API status: ${status ?? "çağrılmadı"})`,
    );
  });

  test("TC-LOG-006: Geçerli email + hatalı şifre → giriş reddedildi", async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
    ...
    await expect(page).toHaveURL(/hesap\/giris/);
    console.log(
      `✓ Hatalı şifre reddedildi (API status: ${status ?? "çağrılmadı"})`,
    );
  });

  test("TC-LOG-007: Hatalı email formatı → giriş yapılmaz", async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
  ...
  test("TC-LOG-008: Hatalı email + hatalı şifre → giriş reddedildi", async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
    ...
    await expect(page).toHaveURL(/hesap\/giris/);
    console.log(
      `✓ İkisi de hatalı, reddedildi (API status: ${status ?? "çağrılmadı"})`,
    );
  });
  ...
  test("TC-LOG-012: Şifre input default olarak password tipinde", async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
  ...
  test('TC-LOG-014: "Üye Olun" linki register sayfasına yönlendirir', async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
  ...
  test('TC-LOG-015: "Parolanızı mı unuttunuz?" linki yönlendirir', async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
  ...
  test('TC-LOG-013: "Beni Hatırla" checkbox DOM\'da mevcut', async ({
    page,
  }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
  ...
  test("TC-LOG-019: Şifre sonu boşluk → giriş reddedildi", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/hesap\/giris/);
    console.log(
      `✓ Şifre boşluklu reddedildi (API status: ${status ?? "çağrılmadı"}) — trim hatası kontrolü`,
    );
  });
});
