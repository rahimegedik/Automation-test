...
- `https://www.nadirgold.work` → `baseURL` (config'ten oku veya process.env.BASE*URL*${ENV})
...
if (env === "prod" && process.env.ALLOW_PROD_ORDERS !== "1") {
  ...
  if (DESTRUCTIVE_SPECS.some((s) => currentSpec.includes(s))) {
    test.skip(
      true,
      `🚫 ${currentSpec} prod'da gerçek sipariş oluşturuyor. ALLOW_PROD_ORDERS=1 ile çalıştır.`,
    );
  }
}
```
