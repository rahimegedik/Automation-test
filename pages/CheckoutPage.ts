...
export class CheckoutPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.bankTransferButton = page.getByRole("button", {
      name: /banka transfer/i,
    });
    ...
    this.addressLabel = page.getByText(
      /gönderim adresi|teslimat adresi|delivery address/i,
    );
    this.paymentHeading = page.getByText(
      /ödeme seçenekleri|ödeme yöntemi|payment method/i,
    );
    this.creditCardButton = page.getByRole("button", {
      name: /banka.*kredi kartı|kredi kartı/i,
    });
    this.savedCardsTab = page.getByRole("button", { name: /kayıtlı kartlar/i });
    this.firstSavedCard = page
      .locator('[class*="card" i], [class*="kart" i]')
      .filter({ hasText: /\d{4}/ })
      .first();
  }
  ...
  async goto() {
    ...
    await this.bankTransferButton
      .or(this.creditCardButton)
      .first()
  ...
  async selectBankTransfer() {
    ...
    await this.bankTransferButton.waitFor({
      state: "visible",
      timeout: 10_000,
    });
